...
import { IOrder, IWatched, STATUS, IOrderInfo } from 'types';
...
export const useOrdersRequest = (props: IProps) => {
  ...
  const loadOrders = useCallback(
    async (status: string[]) => {
      ...
      if (orders) {
        const dates = orders.map((order: IOrder) =>
          dayjs(order.date).format(TIME.DATE)
        );
  ...
  const loadOrdersNotifications = useCallback(
    async (status: STATUS, watched: IWatched) => {
  ...
  const updateOrder = useCallback(
    async (number = '', data: Partial<IOrder>) => {
